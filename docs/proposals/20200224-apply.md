...
The first step in the reconciliation process is to determine the number of nodes
and healthy nodes in the cluster.
...
* are control plane pods running and healthy
* is the node part of the `etcd` ring

#### The Cluster Is Not Provisioned

This case is triggered when the required packages are not present or the `/etc/kubernetes`
directory is absent/empty. This usually happens when newly created VMs are
provided or `kubeone reset` has been run. In this case, `kubeone apply` would just invoke the
installation process (`kubeone install`).
...
the [quorum][etcd-quorum], KubeOne will attempt to automatically repair the cluster.
...
* `etcd` member is not part of the `etcd` ring

In such cases, there is a high chance that rotating the node will fix the problem
and ensure that the cluster can function as expected.

The operator would have to manually remove the non-healthy instance
from (cloud) provider and then update the KubeOne configuration to replace the old
...
installation process (`kubeone install`). This process is explained in the
[manual cluster repair][manual-cluster-repair] document.
...
In the case all nodes are healthy, the reconciliation process compares the actual
cluster status and the expected state. This includes that:
...
cluster or enable/disable features.
...
If one of the requirements are not satisfied, it is up to the operator to repair
the worker nodes.
...
we decide to deprecate the subcommands, a new proposal will be made, with the backwards
