...
func (p *Provider) CleanupLoadBalancer(s *state.State) error {
	...
	if len(describeOut.LoadBalancers) > 0 {
		realLB := describeOut.LoadBalancers[0]
		s.Logger.Debugf("deleting loadbalancer %q with arn: %s", lbName, *realLB.LoadBalancerArn)

		if _, err = client.DeleteLoadBalancer(s.Context, &elasticloadbalancingv2.DeleteLoadBalancerInput{
			LoadBalancerArn: realLB.LoadBalancerArn,
		}); err != nil {
			return fail.Cloud(err, "aws", "deleting loadbalancer")
		}
	}

	tgOut, err := client.DescribeTargetGroups(s.Context, &elasticloadbalancingv2.DescribeTargetGroupsInput{
		Names: []string{awsTargetGroupName(lbName)},
	})
	if err != nil {
		if _, ok := errors.AsType[*elbv2types.TargetGroupNotFoundException](err); !ok {
			return fail.Cloud(err, "aws", "describing target groups")
		}
	}

	if len(tgOut.TargetGroups) > 0 {
		s.Logger.Debugf("deleting target group %q", *tgOut.TargetGroups[0].TargetGroupName)

		if _, err = client.DeleteTargetGroup(s.Context, &elasticloadbalancingv2.DeleteTargetGroupInput{
			TargetGroupArn: tgOut.TargetGroups[0].TargetGroupArn,
		}); err != nil {
			return fail.Cloud(err, "aws", "deleting target group")
		}
	}
