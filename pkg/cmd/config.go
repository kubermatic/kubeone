...
`,
	...
	cmd.Flags().BoolVarP(&pOpts.EnableNodeLocalDNSCache, "enable-node-local-dns-cache", "", true, "enable NodeLocalDNSCache")
...
features:
  ...
  # Opt-out from deploying NodeLocal DNS cache on every node in the cluster.
  # NodeLocal DNS cache is used to prevent the known race conditions.
  # More info:
