...
func Upgrade(s *state.State) error {
	...
	commonSteps := []task.Task{
		...
		{Fn: upgradeControlPlaneNodeBinaries, ErrMsg: "unable to upgrade control plane kubernetes node binaries", Retries: 3},
		...
		{Fn: upgradeWorkers, ErrMsg: "unable to upgrade static worker nodes", Retries: 3},
		{Fn: upgradeStaticWorkerNodeBinaries, ErrMsg: "unable to upgrade static worker kubernetes node binaries", Retries: 3},
