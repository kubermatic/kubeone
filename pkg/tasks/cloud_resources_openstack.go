...
func ensureOpenstackLBMembers(s *state.State) error {
	...
	if poolID == "" {
		...
		discoveredPoolID, err := discoverOpenstackLBPool(lbClient, osCP.LoadBalancer.Name)
		if err != nil {
			return err
		}
...
// withOpenstackEnsureControlPlane returns tasks for creating OpenStack control plane resources.
func withOpenstackEnsureControlPlane(clusterName string, nodeSet []kubeoneapi.NodeSet, kubeletVersion string) (Tasks, error) {
	capimachines, err := generateOpenstackControlPlaneMachines(clusterName, nodeSet, kubeletVersion)
	if err != nil {
		return nil, err
	}

	tasks := generateOpenstackControlPlaneTasks(capimachines)
