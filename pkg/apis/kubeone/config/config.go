...
func SetKubeOneClusterDynamicDefaults(cluster *kubeoneapi.KubeOneCluster, credentialsFilePath string, baseDir string) error {
	...
	if cluster.ControlPlane.NodeSets != nil {
		...
		if err := validation.ValidateCloudProviderSpec(*cluster, field.NewPath("provider")).ToAggregate(); err != nil {
			return fail.ConfigValidation(err)
		}
...
func generateHetznerControlPlaneMachines(nodeSet []kubeoneapi.NodeSet, kubeletVersion string) ([]v1alpha1.Machine, error) {
...
func getOrCreateHetznerLB(cluster *kubeoneapi.KubeOneCluster, credentialsFilePath string) (kubeoneapi.APIEndpoint, error) {
	...
	if len(networks) == 0 {
		return cluster.APIEndpoint, fail.Config(fmt.Errorf("no network ID found with ID: %s\n", networkName), "looking up netzner network")
	}
