...
func (sbb *sonobuoyBin) build(args ...string) *exec.Exec {
	exe := exec.NewExec("sonobuoy",
		exec.WithArgs(args...),
	...
	if sbb.kubeconfig != "" {
		exec.WithEnvs(fmt.Sprintf("KUBECONFIG=%s", sbb.kubeconfig))(exe)
	}
