...
const (
	...
	prowImage             = "quay.io/kubermatic/build:go-1.21-node-18-11"
...
func sonobuoyRunWithRunCount(ctx context.Context, t *testing.T, k1 *kubeoneBin, mode sonobuoyMode, testDir string, runCount int, proxyURL string) {
	...
	if err = retryFn(func() error { return sb.Run(ctx, mode, rerunFailed) }); err != nil {
	...
	if len(report) > 0 {
		...
		if runCount < sonobuoyRunRetries {
			...
			if err = retryFn(func() error { return sb.Delete(ctx) }); err != nil {
