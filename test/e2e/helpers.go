...
func NewSignalContext() context.Context {
	// We use context.WithTimeout because we want to cancel the context before test timeouts. If we allow the test to
	// timeout, the main test process will terminate, but other subprocesses that we call will remain running. For
	// example, we run `sonobuoy wait`-- if it eventually gets stuck, once the test timeouts, the main test process will
	// terminate, but `sonobuoy wait` will continue running until the ProwJob timeout. This also means, because the main
	// process has been terminated, there will be NO cleanup, so we'll leak resources.
	//
	// 55 minutes has been chosen on basis of default test timeout of 1 hour. We allow 5 minutes for tests to clean up
	// after themselves.
	var timeout = 55 * time.Minute

	// Try to parse TEST_TIMEOUT from environment
	if strTimeout, found := os.LookupEnv("TEST_TIMEOUT"); found {
		if maybetimeout, err := time.ParseDuration(strTimeout); err == nil {
			timeout = maybetimeout - 5*time.Minute
		}
	}
